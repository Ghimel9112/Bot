  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      if (args[0]) {
        ...
        .setColor('DARK_RED').setThumbnail(es.thumb ? es.footericon : null)
        .setFooter("Page Overview\n"+ client.user.username, client.user.displayAvatarURL())
        .setTitle(`Information about __${client.user.username}__`)
        .addField(":muscle: **__My Features__**",
`>>> :notes: An advanced <:Spotify:799069730786377759> **Music System** with **Audio Filtering**
:video_game: A unique Music Request System and way much more!
:ballot_box_with_check: Why does the bot needs the administration permission? Because it makes it easier for me to code it and top not get any errors!!`)
...
📶 **\`${Math.floor(client.ws.ping)}ms\` Ping**
  Made by [**Ghimel**](https://discord.io/atac_de_elicoptere)`);
